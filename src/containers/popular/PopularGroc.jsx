...
const PopularGroc = () => {
  ...
  useEffect(() => {
    ...
    hasInitialized.current = true;
      try {
        const response = await axios.get(`${getBaseUrl()}/groceries/popular`);
        setPopularItems(
  ...
  const addToCart_ = (item) => {
    const vendorId = item.vendor_id;
    const vendorItems = cart[vendorId] || [];
    ...
    setCart((prevCart) => {
      ...
      const updatedVendorItems =
        existingItemIndex > -1
          ? currentItems.map((i) => (i.id === item.id ? { ...i, quantity } : i))
    ...
    for (const payload of checkoutPayloads) {
      try {
        const response = await axios.post(
          `${getBaseUrl()}/vendor/make_order`,
          payload
        );
        setOrderSuccess(true);
        console.log(
          `Order for vendor ${payload.vendor_id} successful:`,
          response.data
        );
      } catch (error) {
        console.error(
          `Order for vendor ${payload.vendor_id} failed:`,
